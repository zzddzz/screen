							...
							'<td colspan="2">' +
								'<div class="ui_title_bar">' +
									...
									'<div class="ui_title_buttons">' +
										'<a class="ui_min" href="javascript:void(0);" title="'+$.jQi18lhgdialog.dialogMin+'"><b class="ui_min_b"></b></a>' +
										'<a class="ui_max" href="javascript:void(0);" title="'+$.jQi18lhgdialog.dialogMax+'"><b class="ui_max_b"></b></a>' +
										'<a class="ui_res" href="javascript:void(0);" title="'+$.jQi18lhgdialog.dialogRevert+'"><b class="ui_res_b"></b><b class="ui_res_t"></b></a>' +
										'<a class="ui_close" href="javascript:void(0);" title="">\xd7</a>' +
									'</div>' +
								'</div>' +
							'</td>' +
						'</tr>' +
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		that._winSize = _$top.width() * _$top.height();
		...
		if( config.icon && !isIfr )
		{
			...
			DOM.icon[0].innerHTML = '<img src="' + config.path + 'skins/' + _skin + '/icons/' + config.icon + '" class="ui_icon_bg"/>';
		}
		else
	...
	unlock: function()
	{
		...
		if( mask && that._lock )
		{
			...
			if( config.parent && config.parent._lock )
			{
				...
				//IE9、10 BUG 蒙层会遮盖住弹出框
				config.parent.zindex();
			}
			else
...
lhgdialog.setting =
{
	...
	title: $.jQi18lhgdialog.dialogWindows,     // 标题,默认'视窗'
	button: null,	     		// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数
	init: null,					// 对话框初始化后执行的函数
	close: null,				// 对话框关闭前执行的函数
	okVal: $.jQi18lhgdialog.dialogConfirm,		// 确定按钮文本,默认'确定'
	cancelVal: $.jQi18lhgdialog.dialogCancel,	// 取消按钮文本,默认'取消'
	skin: '',					// 多皮肤共存预留接口
	esc: true,					// 是否支持Esc键关闭
...
{
	return lhgdialog({
		title: $.jQi18lhgdialog.dialogWarn,
		id: 'Alert',
		zIndex: _zIndex(),
		icon: 'alert.gif',
		fixed: true,
		lock: true,
		content: "<div class='msg-info'>"+content+"</div>",
		ok: true,
		resize: false,
		close: callback,
		/*此问题已经通过其他方式解决，这段代码可能会造成运行缓慢，故删除
		 * close:function(){
			callback;
			
			if(parent!=null){
				lhgdialog.focus = parent;
				var wrap = parent.DOM.wrap;
				var button = $(".ui_state_highlight:visible",wrap);
...
{
	return lhgdialog({
		title: $.jQi18lhgdialog.dialogMesseage,
		id: 'Success',
		zIndex: _zIndex(),
		icon: 'success.gif',
		fixed: true,
...
{
	return lhgdialog({
		title: $.jQi18lhgdialog.dialogConfirm,
		id: 'confirm.gif',
		zIndex: _zIndex(),
		icon: 'confirm.gif',
		fixed: true,
...
lhgdialog.prompt = function( content, yes, value, parent )
{
	...
	return lhgdialog({
		title: $.jQi18lhgdialog.dialogPrompt,
		id: 'Prompt',
		zIndex: _zIndex(),
		icon: 'prompt.gif',
		fixed: true,
